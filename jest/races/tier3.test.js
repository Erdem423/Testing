...
const gate = tokenCheck.ok
  ? gateFor("RACE-T3: Metadata races and parallel load", "stripe.customers")
  : skipUnless(tokenCheck, "RACE-T3: Metadata races and parallel load", "Every tier provisions its own Stripe connection and catalog to race against, so it cannot start without a key to create them.");
