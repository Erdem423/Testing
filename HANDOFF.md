...
1. **`COUNT(*)` queries are capped at exactly 100 rows** for live (non-cached) queries, regardless of the table's real size — matches Stripe's default List API page size exactly. Since the `C`/`D` merge this is measured on **all four data tables in a single run**, uncached then cached: `customers` 100→505, `charges` 100→652, `subscriptions` 100→222, `invoices` 100→338. Caching bypasses it completely.
   - **Filtered counts are capped too**, which locates the bug: refunded charges gives 18 live vs 85 cached — 18 of the *first 100 rows*. So the truncation is on the underlying scan, not the aggregate, meaning any `WHERE` clause is silently evaluated against a partial row set. That's broader than `COUNT(*)`.
